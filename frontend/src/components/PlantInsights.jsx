...
function WeatherCard({ plantId }) {
  ...
  const cloudImpact = weather.cloud_cover > 70 ? 'High cloud cover — expect lower generation today' :
    weather.cloud_cover > 40 ? 'Partly cloudy — moderate generation expected' :
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{
        background: weather.cloud_cover > 70
          ? 'rgba(255,87,87,0.1)'
    ...
    <div style={cardStyle}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <div style={{
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700, color: '#fff', marginTop: 4 }}>
            {trees}
          </div>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>trees planted equivalent</div>
        </div>
        <div style={{
          background: 'rgba(0,0,0,0.2)', borderRadius: 8,
          padding: '12px', textAlign: 'center',
          ...
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>cars off road for a year</div>
        </div>
      </div>
    </div>
  )
...
function SavingsCard({ annualKwh }) {
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: '12px', textAlign: 'center' }}>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginBottom: 4 }}>Monthly</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: '#fff' }}>
            ₹{monthly.toLocaleString('en-IN')}
          </div>
        </div>
        <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: '12px', textAlign: 'center' }}>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginBottom: 4 }}>Daily</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: '#fff' }}>
            ₹{daily.toLocaleString('en-IN')}
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        Based on {annualKwh.toLocaleString()} kWh annual forecast × ₹{rate}/kWh
      </div>
    </div>
  )
}
...
export { WeatherCard, CarbonCard, SavingsCard }
